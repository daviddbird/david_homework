...
# file with omd_api_key as the name of the actual key.
# source puts this in to the environment.
source('keys.R')
...
```{r}
response <- GET("http://www.omdbapi.com/?apikey=78207016&?t=Hudsucker+Proxy?")
...
```{r}
my_key = readLines("keys.R")
response <- GET("http://www.omdbapi.com/?apikey=key(my_key)&?s=married+to+the+mob")
http_status(response)
```


query = list(gender = "female", isAlive = TRUE))


```{r}
query <- "http://www.omdbapi.com/?apikey=78207016&?t=Hudsucker+Proxy?"
